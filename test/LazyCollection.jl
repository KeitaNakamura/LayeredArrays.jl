...
@testset "LazyCollection" begin
    ...
    check_collection((@. 2 * layer1)::LazyCollection{1}, 2 * data1)
    check_collection((@. layer1 / 2)::LazyCollection{1}, data1 / 2)
    check_collection((@. layer1 + layer1)::LazyCollection{1}, data1 + data1)
    check_collection((@. layer1 - layer1)::LazyCollection{1}, data1 - data1)
    check_collection((@. layer1 * layer1)::LazyCollection{1}, data1 .* data1)
    ...
    check_collection((@. layer1 * layer2)::LazyCollection{2}, copy(Broadcast.broadcasted(*, Ref(data1), data2)))
    check_collection((@. layer2 * layer1)::LazyCollection{2}, copy(Broadcast.broadcasted(*, data2, Ref(data1))))
    @test_throws Exception layer1 + layer2
    @test_throws Exception layer2 + layer1
    @test_throws Exception layer1 - layer2
    @test_throws Exception layer2 - layer1
    # layer = 0 and 1
    check_collection((@. layer0 * layer1)::LazyCollection{1}, data0 .* data1)
    check_collection((@. layer1 * layer0)::LazyCollection{1}, data1 .* data0)
    check_collection((@. layer0 / layer1)::LazyCollection{1}, data0 ./ data1)
    check_collection((@. layer1 / layer0)::LazyCollection{1}, data1 ./ data0)
    ...
    @test_throws Exception layer1 - layer0
    # layer = 0 and 2
    check_collection((@. layer0 * layer2)::LazyCollection{2}, copy(Broadcast.broadcasted(*, Ref(data0), data2)))
    check_collection((@. layer2 * layer0)::LazyCollection{2}, copy(Broadcast.broadcasted(*, data2, Ref(data0))))
    ...
    x0 = Collection{2}(d0)
    x1 = Collection{2}(d1)
    check_collection((∑(@. 2 * x0 * x1))::LazyCollection{2}, [sum([(2 * d0[i][j] * d1[i][j]) for j in 1:length(d0[i])]) for i in 1:length(d0)])
end
